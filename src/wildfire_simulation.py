...
# Note on Numba Implementation:
# We have incorporated Numba JIT compilation (@jit) to optimize the fire spread probability calculations,
# which is one of the most computationally intensive parts of our simulation. While this follows the
# class learnings about performance optimization, we acknowledge that the implementation could be
# more elegant. The current approach, while functional, makes the code less readable and harder to
# maintain. A more structured approach might have been to create a separate module for optimized
# calculations with better documentation and error handling.

@jit(nopython=True)
def calculate_spread_probabilities(
        grid_size: int, burning_cells: np.ndarray, fire_grid: np.ndarray,
        ignition_points: np.ndarray, vegetation: np.ndarray,
        norm_temp: np.ndarray, norm_humidity: np.ndarray,
        norm_wind_speed: np.ndarray, norm_elevation: np.ndarray,
        wind_direction: np.ndarray, humidity: np.ndarray,
        weights: np.ndarray
) -> np.ndarray:
                ...
                for di in [-1, 0, 1]:
                    for dj in [-1, 0, 1]:
                        ...
                        if (0 <= ni < grid_size and
                            ...
                            # Base probability from vegetation and ignition points
                            base = ignition_points[ni, nj] * vegetation[ni, nj]

                            # Environmental factors influence
                            prob = (
                                    weights[0] * base +  # Base probability weight
                                    weights[1] * norm_temp[ni, nj] +  # Temperature effect
                                    weights[2] * (1 - norm_humidity[ni, nj]) +  # Humidity effect
                                    weights[3] * norm_wind_speed[ni, nj] +  # Wind speed effect
                                    weights[4] * (1 - norm_elevation[ni, nj])  # Elevation effect
                            )

                            # Wind direction influence
                            if di == 0 and dj == 0:
                                wind_influence = 1.0  # No wind effect for same cell
                            else:
                            ...
                            prob *= wind_influence
                            prob *= max(0, 1 - humidity[ni, nj] / 100)  # Humidity dampening
                            spread_prob[ni, nj] = max(prob, 0.01)  # Minimum probability threshold

    return spread_prob


class WildfireSimulation:
    def __init__(self, grid_size: int = 25, max_time_steps: int = 24, convergence_threshold: int = 0.01,
        ...
        shrub_factor = 2.5 if self.hypothesis == 'dense_shrub' else 1
        ...
        self.weather_persistence = 0.9
        self.weather_variability = 0.2
    ...
    def evolve_weather(self) -> None:
        ...
        Note: The autoregressive parameters and weather evolution formulas were developed with ChatGPT assistance due to limited documentation on implementing weather evolution in wildfire simulations.
        ...
        if self.hypothesis == 'high_humidity':
            self.humidity = np.maximum(self.humidity, 95)
    ...
    def simulate_spread(self) -> tuple[np.ndarray, list[float]]:
        ...
        for time_step in range(self.max_time_steps):
            ...
            spread_prob = calculate_spread_probabilities(
                ...
                np.array([0.2, 0.3, 1, 0.7, 0.35])  # weights
            )
...
def monte_carlo_convergence_check(percentages: list[float], window: int = 20, last_n: int = 10,
                                  threshold: float = 0.05) -> tuple[bool, np.ndarray, np.ndarray]:
    """
    ...
    >>> converged, ma, rel,se = monte_carlo_convergence_check(percentages)
    ...
    n = len(percentages)
    std = np.std(percentages, ddof=1)
    se = std / np.sqrt(n)
    ma_threshold = threshold
    se_threshold = 0.1

    ma_stable = (len(rel_changes) >= last_n and
                 all(rel_changes[-last_n:] < ma_threshold))
    se_sufficient = se < se_threshold

    converged = ma_stable and se_sufficient

    return converged, ma, rel_changes, se


def plot_monte_carlo_convergence(percentages: list[float], ma: np.ndarray, window: int = 20,
                                 path: str = 'mc_convergence.png') -> None:
    """
...
def run_single_simulation(hypothesis: str) -> tuple[int, int, float]:
    ...
    sim = WildfireSimulation(hypothesis=hypothesis)
...
def run_simulation(n_runs: int = 1500, hypothesis: str = None, mc_window: int = 20, mc_last_n: int = 10,
                   mc_threshold: float = 0.05) -> list[tuple[int, int, float]]:
    """
    ...
    Note on Parallel Processing Implementation:
    We have implemented parallel processing using Python's multiprocessing module to run multiple
    simulations concurrently, following the class learnings about parallel computing. However, we
    recognize that this implementation, while functional, could be improved. The current approach
    uses a simple Pool of workers without sophisticated error handling or load balancing.
    """
    try:
        ...
        print(f"Standard Error: {se_pct:.3f}%")
...
def hypothesis_testing(control_results: list[tuple[int, int, float]], hypothesis_results: list[tuple[int, int, float]],
...
if __name__ == "__main__":
    try:
        ...
        print("Running Multiple simulation tests:")

        # Control Experiment
        print("Running Control Simulation")
        control_results = run_simulation(n_runs=1500, hypothesis=None)
